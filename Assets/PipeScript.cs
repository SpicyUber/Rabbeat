...
public class PipeScript : MonoBehaviour
{
    ...
    {
        _pipeSpeed += 2;
        if (_pipeSpeed > 10) { _pipeSpeed = 10; }
    ...
    void Update()
    {
        ...
        if (_playerIsIn)
        {
            if(NextStop == null) { EjectPlayer(); } else
            {
                Player.GetComponent<playerScript>().playerBody.velocity= (NextStop.Value.transform.position - Player.transform.position).normalized* _pipeSpeed * Time.deltaTime * 400f;
                if((Player.transform.position - NextStop.Value.transform.position).magnitude < 0.25f) { NextStop = NextStop.Next; }
            }
